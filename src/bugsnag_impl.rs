...
impl Bugsnag {
    ...
    pub fn new(api_key: &str, project_source_dir: &str, notify_url: Option<&str>) -> Bugsnag {
        Bugsnag {
            api_key: api_key.to_owned(),
            device_info: deviceinfo::DeviceInfo::generate(),
            app_info: None,
            project_source_dir: project_source_dir.to_owned(),
            notify_url: notify_url.map(|u| u.to_owned())
        }
    }
...
mod tests {
    ...
    fn test_get_project_dir() {
        let api = Bugsnag::new("api-key", "my-dir", None);
